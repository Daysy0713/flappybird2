...
//SE
    let sound = SKAction.playSoundFileNamed("sound", waitForCompletion: true)
       // スコア用
       var score = 0
...
func setupBird() {
    ...
    func setupItem()  {
        ...
        //アイテムの画像を読み込む
        let itemTexture = SKTexture(imageNamed: "scoreItem")
        ...
        itemNode.physicsBody?.categoryBitMask = self.itemCategory
            ...
            itemNode.zPosition = -50
            
            let item = SKSpriteNode(texture: itemTexture)
            item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2, y: 200)
